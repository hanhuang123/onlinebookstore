...
function CheckItem(obj)
{
	...
	switch(obj.name) {
		...
		case "rePassWord":
			if(obj.value == "") {
				msgBox.innerHTML = "確認密碼不能為空";
				msgBox.className = "error";
				return false;
			} else if(obj.value != document.getElementById("passWord").value) {
				msgBox.innerHTML = "兩次輸入的密碼不相同";
				msgBox.className = "error";
				return false;
			}
			break;
...
function showChater()
{
	...
	_a.onclick = openRoom;
	_a.appendChild(document.createTextNode("線上聊天"));
	_dd.appendChild(_a);
	_dl.appendChild(_dt);
	_dl.appendChild(_dd);
...
{ 
   ...
   J.dialog.get({id: 'haoyue_creat',title: '購買成功',width: 600,height:400, link: '<%=path%>/servlet/CartServlet?id='+id+'&num='+num+'&action=add', cover:true});
}
function add(id)
{
